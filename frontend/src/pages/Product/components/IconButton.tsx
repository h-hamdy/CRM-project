...
const checkBilling = async (id: any) => {
  ...
  try {
    const response = await axios.post(
      "http://localhost:3333/bills/exists",
      factureNumber,
...
export const IconButton = ({ id }: any) => {
  ...
  return billExists ? (
    <Link to={`/Product/${id}`}>
      <div className="inline-block w-6 h-6 border border-gray-300 rounded text-center leading-6 cursor-pointer transition-colors duration-300 hover:bg-gray-200">
        <DiffOutlined />
      </div>
    </Link>
  ) : (
